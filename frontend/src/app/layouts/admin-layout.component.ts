...
@Component({
  ...
  template: `
    ...
    <div class="min-h-screen bg-[var(--background)] text-[var(--foreground)] flex w-full">
      <!-- Admin Sidebar -->
      <aside
        [class]="sidebarOpen() ? 'translate-x-0' : '-translate-x-full md:translate-x-0'"
        class="fixed md:sticky top-0 left-0 h-screen w-64 bg-[var(--surface-elevated)] border-r border-[var(--border)] transition-transform duration-200 ease-in-out z-50 flex flex-col">
        <!-- Sidebar Header -->
      ...
      <div class="flex-1 flex flex-col min-w-0">
        <!-- Admin Header -->
        <header class="sticky top-0 z-40 bg-[var(--surface-elevated)] backdrop-blur-xl border-b border-[var(--border)]">
...
export class AdminLayoutComponent {
  ...
  readonly adminTabs: TabItem[] = [
    ...
    { id: 'discover', label: 'AI Discovery', icon: Sparkles, route: '/admin/discover' },
